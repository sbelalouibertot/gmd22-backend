...
describe('Replace recipe', () => {
    ...
    test('Should correctly replace the recipe event id with another recipe, wich is not already scheduled in the period (1)', async () => {
        ...
        const updatedRecipeEvent = await prisma.recipeEvent.findFirst({ where: { id: ids.recipeEvent_1 } }) // findUnique causes a timeout
    ...
    test('Should correctly replace the recipe event id with another recipe, wich is not already scheduled in the period (2)', async () => {
        ...
        const updatedRecipeEvent = await prisma.recipeEvent.findFirst({ where: { id: 'recipe_event_2' } })
    ...
    test('Should not change anything if the recipeEvent is scheduled in another period', async () => {
        ...
        await seedEvent(prisma, { id: 'event_6', type: 'PREPARATION', date: new Date("2022-01-10T13:00:00.000Z"), user: { connect: { id: ids.user_1 } } },
        )
        await seedRecipeEvent(prisma, { id: 'recipe_event_3', event: { connect: { id: 'event_6' } }, recipe: { connect: { id: 'recipe_3' } } })
