...
describe('Get events', () => {
    ...
    const isOnSameMonthAndYear = (date, year: number, month: number): boolean => new Date(date).getUTCFullYear() === year &&
        new Date(date).getUTCMonth() === month - 1

    const isOnSameDay = (date, year: number, month: number, day: number): boolean => isOnSameMonthAndYear(date, year, month) &&
    ...
    test('Only the events of another user should not be returned', async () => {
        ...
        await seedEvent(prisma, { id: 'event_7', type: 'PERIOD_START', date: new Date("2022-01-01T00:00:00.000Z"), user: { connect: { id: 'user_2' } } })
        ...
        expect(allEventsWithAnotherUser).toEqual(
            expect.arrayContaining(
                eventsData.map(() => expect.objectContaining({ id: 'event_7' }))
            )
        )
    })
    ...
    test('Only the events of the right date should be returned', async () => {
        const allEvents = (await getEvents(prisma, { userId: ids.user_1, date: new Date('2022-01-02') })).events

        expect(allEvents.length).toBe(eventsData.filter(event =>
    ...
    test('Only the events of the current period should be returned', async () => {
        const allEvents = (await getEvents(prisma, { userId: ids.user_1, onlyCurrentPeriod: true })).events

        expect(allEvents.length).toBe(eventsData.filter(event =>
            isOnSameMonthAndYear(event.date, 2022, 1) &&
            new Date(event.date).getUTCDate() >= 1 && new Date(event.date).getUTCDate() < 7
        ).length)

        for (const event of allEvents) {
            expect(isOnSameMonthAndYear(event.date, 2022, 1) &&
                new Date(event.date).getUTCDate() >= 1 && new Date(event.date).getUTCDate() < 7).toBeTruthy()
        }
    })

    test('Only the events of the user, with the right date and the right type should be returned', async () => {
        const newEvent: Prisma.EventCreateInput = { id: 'event_10', type: 'PREPARATION', date: new Date("2022-01-02T21:00:00.000Z"), user: { connect: { id: ids.user_1 } } }
        await seedEvent(prisma, newEvent)
        const allEvents = (await getEvents(prisma, { userId: ids.user_1, date: new Date("2022-01-02"), type: 'PREPARATION' })).events

        expect(allEvents.length).toBe([...eventsData, newEvent].filter(event =>
            isOnSameDay(event.date, 2022, 1, 2) &&
            event.type === 'PREPARATION'
        ).length)

        for (const event of allEvents) {
            expect(isOnSameDay(event.date, 2022, 1, 2) &&
                event.type === 'PREPARATION' && [...eventsData, newEvent].map(eventData => eventData.id).includes(event.id)).toBeTruthy()
        }
    })
})