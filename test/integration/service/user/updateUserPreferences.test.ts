...
import { shutdown, clear, seedUser, seedUserPreference } from '../../../../prisma/seed/seed'
...
describe('Update user preferences', () => {
    ...
    const userPreferencesData: Prisma.UserPreferenceCreateInput[] = [
        { id: ids.userPreference_1, user: { connect: { id: ids.user_1 } }, type: 'MAX_RECIPES_PER_WEEK', value: 5 },
        { id: 'user_preference_2', user: { connect: { id: ids.user_1 } }, type: 'SHOPPING_WEEKS_INTERVAL', value: 3 },
    ]
    ...
    test('Should correctly update user preference of the right user (1)', async () => {
        const initialUserPreferences = await prisma.userPreference.findMany({ where: { userId: ids.user_1 } })
        expect(initialUserPreferences).toEqual(
            expect.arrayContaining(
                userPreferencesData.map(userPreferenceData => expect.objectContaining(omit(userPreferenceData, 'user')))
            )
        )
        ...
        const newUserPreferences = await prisma.userPreference.findMany({ where: { userId: ids.user_1 } })
        expect(newUserPreferences).toEqual(
            expect.arrayContaining(
                newUserPreferencesInput.map(userPreferenceData => expect.objectContaining(userPreferenceData))
            )
        )
    })

    test('Should correctly update user preference of the right user (2)', async () => {
        const userPreference: Prisma.UserPreferenceCreateInput = { id: 'user_preference_3', type: 'SHOPPING_WEEKS_INTERVAL', value: 1, user: { connect: { id: 'user_2' } } }
        await seedUserPreference(prisma, userPreference)

        const initialUserPreferences = await prisma.userPreference.findMany({ where: { userId: 'user_2' } })
