...
export default {
  RecipeInstruction: {
    foodItems: async (
      ...
      const parentRecipeInstruction = await ctx.prisma.recipeInstruction.findUnique({ where: { id: parent.id }, include: { recipeInstructionFoods: { include: { food: true } } } })
      if (!parentRecipeInstruction) return []
